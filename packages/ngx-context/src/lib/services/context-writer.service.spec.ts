...
const injectService = (cb) =>
  inject([ContextWriterService], (contextService: ContextWriterService) =>
    cb(contextService)
  );
...
describe('The Context Writer Service', () => {
  ...
  describe('Setting the title', () => {
    it(
      ...
      injectService((contextService: ContextWriterService) => {
        contextService.setTag('title', {});
        expect(document.head.querySelector('title').innerText).toBe(
          metaConfig.defaults.title
        );
      })
    );
  ...
  describe('Setting tags', () => {
    ...
    it(
      'should use the default value if no content was provided',
      injectService((contextService: ContextWriterService) => {
    ...
    // it(
    //   'should empty the tag if no value or default value was provided',
    //   injectService((contextService: ContextWriterService) => {
    //     contextService.setTag('description');
    //     expect(
    //       document.head
  ...
  describe('Updating the metatags', () => {
    it(
      'should not update metatags if disableUpdate is set to true',
      injectService((contextService: ContextWriterService) => {
    ...
    it(
      'should extend the title if extendTitle is true and there is a parent',
      injectService((contextService: ContextWriterService) => {
        const testMeta = {
          title: 'test',
          parent: ['parent'],
        ...
        expect(document.head.querySelector('title').innerText).toEqual(
          'test, parent'
        );
      })
    );
