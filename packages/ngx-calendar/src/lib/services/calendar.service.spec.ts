...
const injectService = (cb) => {
  return inject([CalendarService], (calendarService: CalendarService) =>
    cb(calendarService)
  );
};
...
describe('The Calendar Service', () => {
  ...
  describe('getMonth', () => {
    it(
      'returns the stored month if the year hasnt changed',
    ...
    it(
      'clears the stored months if the provided year differs from the stored year',
      injectService((calendarService) => {
    ...
    it(
      'generates a new month if it was not found in the cache and stores it in the cache',
      injectService((calendarService) => {
    ...
    it(
      'generates a new month for the provided year',
      injectService((calendarService) => {
        const months = calendarService.getMonth(1, 2018);

        expect(months.length).toBe(5);
        expect(calendarService.months).toEqual({
          1: jasmine.arrayContaining([jasmine.arrayContaining([{ date: 12 }])]),
        });
      })
    );
  });
  ...
  describe('getMonthForDate', () => {
    it(
      'calls the getMonth method for the provided date',
      injectService((calendarService) => {
  ...
  describe('getRangeForDate', () => {
    it(
      'generates a range from the provided date and range',
      injectService((calendarService) => {
        const requestedDate = new Date('2018-10-03');
  ...
  describe('getRangesForDate', () => {
    it(
      'generates a range for the provided date, the month before and the month after',
      injectService((calendarService) => {
  ...
  describe('getClosestDateForRange', () => {
    it(
      'gets the closest date for now if no date was provided',
      injectService((calendarService) => {
    ...
    it(
      ...
      injectService((calendarService) => {
        ...
        expect(calendarService.getClosestDateForRange(requestedDate, [4])).toBe(
          requestedDate
        );
      })
    );
    ...
    it(
      ...
      injectService((calendarService) => {
        ...
        expect(
          calendarService.getClosestDateForRange(requestedDate, [5, 6])
        ).toEqual(now);
      })
    );
  });
